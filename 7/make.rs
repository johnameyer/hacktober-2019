...
fn main() -> std::io::Result<()> {
    ...
    if let Some(unwrapped_tasks) = tasks {
        ...
        if let Some(default) = first_task {
            let mut done: HashMap<String, bool> = HashMap::new();
            run(&default, &declared_tasks, &mut done);
        } else {
...
fn run(task: &String, declared_tasks: &HashMap<String, Task>, done: &mut HashMap<String, bool>) -> std::io::Result<()> {
    if let Some(task_status) = done.get(task) {
        if !task_status {
            println!("Ignoring circular dependency");
        }
        return Ok(());
    }
...
fn should_run(task: &Task, declared_tasks: &HashMap<String, Task>) -> bool {
